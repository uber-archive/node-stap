...
function readConsString(stringPtr, depth) {
    ...
    var secondString = self.getStringInternal(second, depth + 1);
...
function readFuncSharedInfoName(sharedInfoPtr) {
    ...
    var name = self.readString(ptr);

    if (name.length === 0) {
        ptr = self.readPointer(sharedInfoPtr + SHARED_INFO_INFERRED_NAME_OFFSET);
        name = self.readString(ptr);
    }
 
    // console.log('name_ptr:', ptr);
...
function readFunction(funcPtr) {
    ...
    var functionName = self.readFuncSharedInfoName(sharedInfoPtr);
